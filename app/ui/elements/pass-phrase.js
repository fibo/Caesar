...
class PassPhrase extends HTMLElement {
  ...
  numWords = '1'
  numWordsRange = document.createElement('input')
  numWordsValue = document.createElement('span')
  ...
  connectedCallback() {
    ...
    numWordsLabel.textContent = 'number of words'

    numWordsRange.type = 'range'
    numWordsRange.min = '1'
    numWordsRange.max = '10'
    numWordsRange.value = numWords

    numWordsValue.textContent = numWords
    ...
    subscribe('CRYPT_DIRECTION', (direction) => {
      if (direction === 'encrypt') {
        ...
        numWordsContainer.hidden = false
      }
      if (direction === 'decrypt') {
        generateButton.hidden = true
        numWordsContainer.hidden = true
      }
    })
  ...
  handleEvent(event) {
    ...
    if (event.type === 'input' && event.target === this.numWordsRange) {
      const range = /** @type {HTMLInputElement} */ (event.target)
      this.numWords = range.value
      this.numWordsValue.textContent = this.numWords
      this.generatePassphrase()
    }
  ...
  generatePassphrase() {
    const randomWords = Array.from({ length: +this.numWords }, () => {
